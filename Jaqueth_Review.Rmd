...
Hi Daniel,
Impressive work! I learned a lot from reading through your script. You gave me some new ideas to use for my future scripting.

I had some initial difficulty getting the script to run past line 63, but I'm sure that was due to my ignorance in using R-studio. I was doing "Run All", but it only went to line 63. I solved it by adding an "r" between all the ```{} brackets at the beginning of each section. After I made that change, it ran fully to the end of the script.

I liked your progress checks, like in line 78. I'm going to do that in the future.

You could use a "for"loop to write the files if you wanted. You could also make a results folder and write them all to that folder.

I admire your ability to work with headers, renaming them and transposing them. Spot on.
...
Really intersting last graph with the distribution of markers across chromosomes. I didn't realize there was anything odd about this set of marker data until you plotted it out like this. The Fang et al dataset subsetted marker data the Maize_50K_SNP_chip in a non-random way. They overselected some region and underselected other regions. You gave a nice way of looking at these data
...
SNP <- read_tsv("https://raw.githubusercontent.com/dkohlhase/BCB546X_Unix_Assignment/master/snp_position.txt")   ##JJ: I didn't know you could read from a website. Super cool
...
```{r Processing full data}
tgeno <- as.data.frame(t(geno))
names(tgeno) <- lapply(tgeno[1, ], as.character)
tgeno <- tgeno[-1,]
tgeno <- rownames_to_column(tgeno, var="SNP_ID")
#write_tsv(tgeno, "./tgeno.txt")  ####Added by Jen to QC running of script
mgeno <- merge(sSNP,tgeno, by.x="SNP_ID", by.y="SNP_ID", all = TRUE)
#write_tsv(mgeno, "./mgeno.txt")  ####Added by Jen to QC running of script
#write_tsv(mgeno, "mgeno-test2.txt")  ####Added by Jen to QC running of script
```
### Maize
...
#write_csv(maize_chrom10_increase, "maize_chrom10_increase-test.txt")  ####Added by Jen to QC running of script
```
...
colnames(zygotes)[4:5] <- c("SNP_ID", "Homozygous")  ##JJ: I like how you make everything tidy by adding header names
colnames(zygotes)
```
...
ggsave(filename = "SNPs per chromosome.png", device = "png")  ##JJ: I didn't realize you could save these as files. Nice.
```
### SNPs per Group
```{r}
ggplot(data = geno) + 
